...
import com.as.service.hims.AccountEntry_Receipt_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/account-receipt")
@CrossOrigin(origins = "*", maxAge = 3600)
public class AccountEntry_Receipt_Controller {

	@Autowired
	AccountEntry_Receipt_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody AccountEntry_Receipt_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody AccountEntry_Receipt_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
		return service.update(form,yearCd, principal);
	}
	...
	public BaseResponse delete(@PathVariable(value = "id") long id, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		...
		return service.delete(id,yearCd, principal);
	}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/findByIpdno/{ipdno}")
	public BaseResponse findByIpdno(@PathVariable(value = "ipdno") String ipdno,Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.findByIpdno(ipdno,principal,yearCd);
	}
	...
	public HttpEntity<byte[]> printReport(@PathVariable("format") String format, @PathVariable(value = "uhid") String uhid, @RequestHeader("yearCd") Long yearCd) {
		final byte[] data = service.report_PDF(uhid,yearCd,format);
