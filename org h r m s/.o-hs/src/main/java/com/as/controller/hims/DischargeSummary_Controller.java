...
import com.as.service.hims.DischargeSummary_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/ipd-discharge-summary")
@CrossOrigin(origins = "*", maxAge = 3600)
public class DischargeSummary_Controller {
	
	@Autowired
	DischargeSummary_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody DischargeSummary_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody DischargeSummary_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/printReport/{format}/{id}")
	@ResponseBody
	public HttpEntity<byte[]> printReport(@PathVariable("format") String format, @PathVariable(value = "id") String id, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		final byte[] data = service.report_PDF(id,format,principal);
