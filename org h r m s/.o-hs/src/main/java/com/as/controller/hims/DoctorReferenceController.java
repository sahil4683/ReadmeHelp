...
import com.as.service.hims.DoctorReference_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/doctor_reference")
@CrossOrigin(origins = "*", maxAge = 3600)
public class DoctorReferenceController {

	@Autowired
	DoctorReference_Service service;

	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody DoctorReference_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody DoctorReference_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/getDoctorReferenceList")
	public BaseResponse getDoctorReferenceList(Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getDoctorReferenceList(principal);
	}
