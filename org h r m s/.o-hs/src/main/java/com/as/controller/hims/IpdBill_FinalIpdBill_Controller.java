...
import com.as.service.hims.IpdBill_FinalIpdBill_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/finalipd-bill")
@CrossOrigin(origins = "*", maxAge = 3600)
public class IpdBill_FinalIpdBill_Controller {
	
	
	@Autowired
	IpdBill_FinalIpdBill_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody IpdBill_FinalIpdBill_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody IpdBill_FinalIpdBill_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/getLabRequest/{ipdno}/{type}")
	public BaseResponse getLabRequest(@PathVariable(value = "ipdno") String ipdno,@PathVariable(value = "type") String type, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getLabRequest(ipdno,type, principal,yearCd);
	}
	...
	public HttpEntity<byte[]> printReport(
			...
			Principal principal, @RequestHeader("yearCd") Long yearCd) {
		final byte[] data = service.report_PDF(id,format,type,principal,yearCd);
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/getReadyForDischarge")
	public ResponseEntity<BaseResponse> getReadyForDischarge(@RequestHeader("yearCd") Long yearCd){
		BaseResponse response = service.getReadyForDischarge(yearCd);
