...
import com.as.service.hims.MasterAppointmentMaster_AppointmentScheduler_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-appointment_master-appointment_scheduler")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterAppointmentMaster_AppointmentScheduler_Controller {

	@Autowired
	MasterAppointmentMaster_AppointmentScheduler_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterAppointmentMaster_AppointmentScheduler_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterAppointmentMaster_AppointmentScheduler_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
