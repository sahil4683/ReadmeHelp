...
import com.as.service.hims.MasterOtherMasters1_CasepaperMedicineMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-other_masters1-casepaper_medicine_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterOtherMasters1_CasepaperMedicineMaster_Controller {

	@Autowired
	MasterOtherMasters1_CasepaperMedicineMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterOtherMasters1_CasepaperMedicineMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterOtherMasters1_CasepaperMedicineMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
