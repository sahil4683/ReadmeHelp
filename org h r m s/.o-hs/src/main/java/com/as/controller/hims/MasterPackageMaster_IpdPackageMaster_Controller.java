...
import com.as.service.hims.MasterPackageMaster_IpdPackageMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-package_master-ipd_package_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterPackageMaster_IpdPackageMaster_Controller {
	
	@Autowired
	MasterPackageMaster_IpdPackageMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterPackageMaster_IpdPackageMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterPackageMaster_IpdPackageMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	public BaseResponse getDetailsById(@PathVariable(value = "id") String id, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		...
		return service.getDetailsById(id, principal);
	}
