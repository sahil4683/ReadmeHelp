...
import com.as.service.hims.MasterTestMaster_IpdTest_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-test_master-ipd_test")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterTestMaster_IpdTest_Controller {

	@Autowired
	MasterTestMaster_IpdTest_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterTestMaster_IpdTest_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterTestMaster_IpdTest_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/getParticularsListByGroup/{groupName}/{org}")
	public BaseResponse getParticularsListByGroup(@PathVariable(value = "groupName") String groupName,@PathVariable(value = "org") String org, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getParticularsListByGroup(groupName,org, principal);
	}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/getDailyTestList")
	public BaseResponse getDailyTestList(Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getDailyTestList(principal);
	}
