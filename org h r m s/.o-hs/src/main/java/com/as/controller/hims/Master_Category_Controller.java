...
import com.as.service.hims.Master_Category_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master_category")
@CrossOrigin(origins = "*", maxAge = 3600)
public class Master_Category_Controller {

	@Autowired
	Master_Category_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody Master_Category_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody Master_Category_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	public BaseResponse getCategoryList(@PathVariable(value = "categoryType") String categoryType, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getCategoryList(categoryType, principal);
	}
