...
import com.as.service.hims.OrganizationMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/organization_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class OrganizationMasterController {
	
	@Autowired
	OrganizationMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody OrganizationMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody OrganizationMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	public BaseResponse getOrganizationList(Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getOrganizationList(principal);
	}
}
