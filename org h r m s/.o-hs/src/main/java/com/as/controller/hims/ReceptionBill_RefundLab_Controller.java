...
import com.as.service.hims.ReceptionBill_RefundLab_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/reception-bill_refund-lab")
@CrossOrigin(origins = "*", maxAge = 3600)
public class ReceptionBill_RefundLab_Controller {
	
	@Autowired
	ReceptionBill_RefundLab_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody ReceptionBill_RefundLab_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody ReceptionBill_RefundLab_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
