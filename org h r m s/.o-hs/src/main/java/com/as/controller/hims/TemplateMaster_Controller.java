...
import com.as.service.hims.TemplateMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/template_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class TemplateMaster_Controller {
	
	@Autowired
	TemplateMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody TemplateMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody TemplateMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/findByTemplate/{template}")
	public BaseResponse findByTemplate(@PathVariable(value = "template") String template,Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.findByTemplate(template,principal);
	}
