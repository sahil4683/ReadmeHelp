...
import com.as.service.patho.Patho_SampleDeviceMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/patho_sample_device_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class Patho_SampleDeviceMaster_Controller {
	
	@Autowired
	Patho_SampleDeviceMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody Patho_SampleDeviceMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody Patho_SampleDeviceMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
