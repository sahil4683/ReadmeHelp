...
import com.as.service.patho.Patho_StatusMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/patho_status_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class Patho_StatusMaster_Controller {
	
	@Autowired
	Patho_StatusMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody Patho_StatusMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody Patho_StatusMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
