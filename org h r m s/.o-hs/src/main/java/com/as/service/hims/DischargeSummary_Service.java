...
@Service
public class DischargeSummary_Service {

	@Autowired
	DischargeSummary_Repository repository;
	...
	public BaseResponse save(DischargeSummary_DTO form, Principal principal,Long creditYear) {
		BaseResponse response = new BaseResponse();
		DischargeSummary_Entity entity = repository.findByIpdnoAndUhidAndCreditYear(form.getIpdno(),form.getUhid(), creditYear);
		if (entity != null) {
...
//		DischargeSummary_Entity entity = new DischargeSummary_Entity();
		entity = new DischargeSummary_Entity();
		...
		entity.setLastupdate(principal.getName());
	...
	public BaseResponse update(DischargeSummary_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		DischargeSummary_Entity entity = repository.findById(form.getId());
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
	...
	public BaseResponse delete(long id, Principal principal) {
		BaseResponse response = new BaseResponse();
		DischargeSummary_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
	...
	public byte[] report_PDF(String id, String format,Principal principal) {
		try {			
			String templetSrc = env.getProperty("ipd.discharge_summary.file");
			
			DischargeSummary_Entity summList = repository.findById(Long.valueOf(id));
			...
			parameters.put("summary3", summList.getSummary3());
			parameters.put("summary4", summList.getSummary4());
			parameters.put("summary5", summList.getSummary5());
			parameters.put("summary6", summList.getSummary6());
			parameters.put("summary7", summList.getSummary7());
			parameters.put("summary8", summList.getSummary8());
			parameters.put("summary9", summList.getSummary9());
			parameters.put("summary10", summList.getSummary10());
			parameters.put("summary11", summList.getSummary11());
			parameters.put("summary12", summList.getSummary12());
			parameters.put("summary13", summList.getSummary13());
			parameters.put("summary14", summList.getSummary14());
