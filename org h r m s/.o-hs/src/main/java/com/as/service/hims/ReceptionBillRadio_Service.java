...
import com.as.h_dto.hims.ReceptionBillRadio_DTO;
import com.as.h_dto.hims.ReceptionBillRadio_Details_DTO;
...
import com.as.reporsitories.hims.ReceptionBillRadio_Details_Repository;
...
@Service
public class ReceptionBillRadio_Service {

	@Autowired
	ReceptionBillRadio_Details_Repository details_Repository;

	@Autowired
	ReceptionBillRadio_Repository repository;
	...
	public BaseResponse save(ReceptionBillRadio_DTO form, Long yearCd, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBillRadio_Entity entity = repository.findByBillNoAndCreditYear(form.getBillNo(), yearCd);
		if(entity !=null) {
		...
		entity = new ReceptionBillRadio_Entity();
		 ...
		 */
		// For Bill Number
		NumberSequenceUtility utility = sequenceUtilityRepository.findByName("Radio Bill");
		...
		for (ReceptionBillRadio_Details_DTO detail : form.getDetailsList()) {
			ReceptionBillRadio_Details_Entity details_Entity = new ReceptionBillRadio_Details_Entity();
	...
	public BaseResponse update(ReceptionBillRadio_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBillRadio_Entity entity = repository.findById(form.getId());
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
		...
		for (ReceptionBillRadio_Details_DTO detail : form.getDetailsList()) {
			ReceptionBillRadio_Details_Entity details_Entity = details_Repository.findById(detail.getId());
			if (details_Entity != null) {
				...
				details_Entity = new ReceptionBillRadio_Details_Entity();
				BeanUtils.copyProperties(detail, details_Entity);
				details_Entity
						.setGroupName(group_Repository.findById(Long.valueOf(detail.getGroupName())).getGroupName());
				if (detail.getProcedureDoctor1() != null)
	...
	public BaseResponse delete(long id, Principal principal,Long creditYear) {
		BaseResponse response = new BaseResponse();
		ReceptionBillRadio_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			List<ReceptionBillRadio_Details_Entity> delete = details_Repository
	...
	public BaseResponse getDetailsById(Long id, Principal principal,Long creditYear) {
		...
		ReceptionBillRadio_DTO dto = new ReceptionBillRadio_DTO();
		List<ReceptionBillRadio_Details_DTO> detailsList = new ArrayList<>();

		ReceptionBillRadio_Entity entity = repository.findById(id);
		BeanUtils.copyProperties(entity, dto);

		List<ReceptionBillRadio_Details_Entity> detail = details_Repository
				.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
		for (ReceptionBillRadio_Details_Entity detail_temp : detail) {
			ReceptionBillRadio_Details_DTO temp = new ReceptionBillRadio_Details_DTO();
	...
	public BaseResponse deleteDetailById(Long id, Principal principal) {
		...
		response.setType(ResponseType.SUCCESS);
		response.setMessage("List");
		ReceptionBillRadio_Details_Entity entity = details_Repository.findById(id);
	...
	public BaseResponse getDetailsByBillNo(String billNo, Principal principal,Long creditYear) {
		...
		ReceptionBillRadio_DTO dto = new ReceptionBillRadio_DTO();
		List<ReceptionBillRadio_Details_DTO> detailsList = new ArrayList<>();
		ReceptionBillRadio_Entity entity = repository.findByBillNoAndCreditYear(billNo,creditYear);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
	...
	public byte[] report_PDF(String uhid, String format,Long creditYear) {
		try {
			...
			ReceptionBillRadio_Entity dataList = repository.findById(Long.valueOf(uhid));
			ReceptionReceptionRegistration_Entity registration= registration_Repository.findByUHID(dataList.getUhid());
			List<ReceptionBillRadio_Details_Entity> tempList = details_Repository.findByBillNoAndCreditYear(String.valueOf(dataList.getId()),creditYear);
			...
			parameters.put("billType", "RADIO BILL");
