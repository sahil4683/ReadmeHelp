...
import com.as.h_dto.hims.ReceptionBill_RefundOpd_DTO;
import com.as.h_dto.hims.ReceptionBill_RefundOpd_Details_DTO;
import com.as.h_entities.hims.ReceptionBill_RefundOpd_Details_Entity;
import com.as.h_entities.hims.ReceptionBill_RefundOpd_Entity;
...
import com.as.reporsitories.hims.ReceptionBill_RefundOpd_Details_Repository;
...
@Service
public class ReceptionBill_RefundOpd_Service {

	@Autowired
	ReceptionBill_RefundOpd_Details_Repository details_Repository;

	@Autowired
	ReceptionBill_RefundOpd_Repository repository;
	...
	public BaseResponse save(ReceptionBill_RefundOpd_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBill_RefundOpd_Entity entity = null;

		entity = new ReceptionBill_RefundOpd_Entity();
		...
		entity.setOpdBillNo(form.getBillNo());
		
		// For Bill Number
		NumberSequenceUtility utility = sequenceUtilityRepository.findByName("OPD Bill Refund");
		...
		for (ReceptionBill_RefundOpd_Details_DTO detail : form.getDetailsList()) {
			ReceptionBill_RefundOpd_Details_Entity details_Entity = new ReceptionBill_RefundOpd_Details_Entity();
	...
	public BaseResponse update(ReceptionBill_RefundOpd_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBill_RefundOpd_Entity entity = repository.findById(form.getId());
		if (entity == null) {
		...
		for (ReceptionBill_RefundOpd_Details_DTO detail : form.getDetailsList()) {
			ReceptionBill_RefundOpd_Details_Entity details_Entity = details_Repository.findById(detail.getId());
			if (details_Entity != null) {
				...
				details_Entity = new ReceptionBill_RefundOpd_Details_Entity();
				BeanUtils.copyProperties(detail, details_Entity);
				details_Entity
						.setGroupName(group_Repository.findById(Long.valueOf(detail.getGroupName())).getGroupName());
				if (detail.getProcedureDoctor1() != null)
	...
	public BaseResponse delete(long id, Principal principal,Long creditYear) {
		BaseResponse response = new BaseResponse();
		ReceptionBill_RefundOpd_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			List<ReceptionBill_RefundOpd_Details_Entity> delete = details_Repository
	...
	public BaseResponse getDetailsById(Long id, Principal principal,Long creditYear) {
		...
		ReceptionBill_RefundOpd_DTO dto = new ReceptionBill_RefundOpd_DTO();
		List<ReceptionBill_RefundOpd_Details_DTO> detailsList = new ArrayList<>();

		ReceptionBill_RefundOpd_Entity entity = repository.findById(id);
		BeanUtils.copyProperties(entity, dto);

		List<ReceptionBill_RefundOpd_Details_Entity> detail = details_Repository.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
		for (ReceptionBill_RefundOpd_Details_Entity detail_temp : detail) {
			ReceptionBill_RefundOpd_Details_DTO temp = new ReceptionBill_RefundOpd_Details_DTO();
	...
	public BaseResponse deleteDetailById(Long id, Principal principal) {
		...
		response.setType(ResponseType.SUCCESS);
		response.setMessage("List");
		ReceptionBill_RefundOpd_Details_Entity entity = details_Repository.findById(id);
	...
	public byte[] report_PDF(String uhid, String format,Long creditYear) {
		try {
			...
			ReceptionBill_RefundOpd_Entity dataList = repository.findById(Long.valueOf(uhid));
			ReceptionReceptionRegistration_Entity registration= registration_Repository.findByUHID(dataList.getUhid());
			List<ReceptionBill_RefundOpd_Details_Entity> tempList = details_Repository.findByBillNoAndCreditYear(String.valueOf(dataList.getId()),creditYear);
			...
			parameters.put("billType", "Financial Credit Note OPD");
