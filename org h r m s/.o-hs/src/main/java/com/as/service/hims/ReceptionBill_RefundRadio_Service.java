...
import com.as.h_dto.hims.ReceptionBill_RefundRadio_DTO;
import com.as.h_dto.hims.ReceptionBill_RefundRadio_Details_DTO;
import com.as.h_entities.hims.ReceptionBill_RefundRadio_Details_Entity;
import com.as.h_entities.hims.ReceptionBill_RefundRadio_Entity;
...
import com.as.reporsitories.hims.ReceptionBill_RefundRadio_Details_Repository;
...
@Service
public class ReceptionBill_RefundRadio_Service {

	@Autowired
	ReceptionBill_RefundRadio_Details_Repository details_Repository;

	@Autowired
	ReceptionBill_RefundRadio_Repository repository;
	...
	public BaseResponse save(ReceptionBill_RefundRadio_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBill_RefundRadio_Entity entity = null;

		entity = new ReceptionBill_RefundRadio_Entity();
		...
		entity.setRadioBillNo(form.getBillNo());
		
		// For Bill Number
		NumberSequenceUtility utility = sequenceUtilityRepository.findByName("Radio Bill Refund");
		...
		for (ReceptionBill_RefundRadio_Details_DTO detail : form.getDetailsList()) {
			ReceptionBill_RefundRadio_Details_Entity details_Entity = new ReceptionBill_RefundRadio_Details_Entity();
	...
	public BaseResponse update(ReceptionBill_RefundRadio_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBill_RefundRadio_Entity entity = repository.findById(form.getId());
		if (entity == null) {
		...
		for (ReceptionBill_RefundRadio_Details_DTO detail : form.getDetailsList()) {
			ReceptionBill_RefundRadio_Details_Entity details_Entity = details_Repository.findById(detail.getId());
			if (details_Entity != null) {
				...
				details_Entity = new ReceptionBill_RefundRadio_Details_Entity();
				BeanUtils.copyProperties(detail, details_Entity);
				details_Entity
						.setGroupName(group_Repository.findById(Long.valueOf(detail.getGroupName())).getGroupName());
				if (detail.getProcedureDoctor1() != null)
	...
	public BaseResponse delete(long id, Principal principal,Long creditYear) {
		BaseResponse response = new BaseResponse();
		ReceptionBill_RefundRadio_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			List<ReceptionBill_RefundRadio_Details_Entity> delete = details_Repository
	...
	public BaseResponse getDetailsById(Long id, Principal principal,Long creditYear) {
		...
		ReceptionBill_RefundRadio_DTO dto = new ReceptionBill_RefundRadio_DTO();
		List<ReceptionBill_RefundRadio_Details_DTO> detailsList = new ArrayList<>();

		ReceptionBill_RefundRadio_Entity entity = repository.findById(id);
		BeanUtils.copyProperties(entity, dto);

		List<ReceptionBill_RefundRadio_Details_Entity> detail = details_Repository
				.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
		for (ReceptionBill_RefundRadio_Details_Entity detail_temp : detail) {
			ReceptionBill_RefundRadio_Details_DTO temp = new ReceptionBill_RefundRadio_Details_DTO();
	...
	public BaseResponse deleteDetailById(Long id, Principal principal) {
		...
		response.setType(ResponseType.SUCCESS);
		response.setMessage("List");
		ReceptionBill_RefundRadio_Details_Entity entity = details_Repository.findById(id);
	...
	public byte[] report_PDF(String uhid, String format,Long creditYear) {
		try {
			...
			ReceptionBill_RefundRadio_Entity dataList = repository.findById(Long.valueOf(uhid));
			ReceptionReceptionRegistration_Entity registration= registration_Repository.findByUHID(dataList.getUhid());
			List<ReceptionBill_RefundRadio_Details_Entity> tempList = details_Repository.findByBillNoAndCreditYear(String.valueOf(dataList.getId()),creditYear);
			...
			parameters.put("billType", "Financial Credit Note RADIO");
