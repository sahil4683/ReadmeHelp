...
import com.as.service.hims.AccountEntry_Payment_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/account-payment")
@CrossOrigin(origins = "*", maxAge = 3600)
public class AccountEntry_Payment_Controller {

	@Autowired
	AccountEntry_Payment_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody AccountEntry_Payment_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody AccountEntry_Payment_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
