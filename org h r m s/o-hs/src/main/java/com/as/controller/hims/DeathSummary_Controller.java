...
import com.as.service.hims.DeathSummary_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/ipd-death-summary")
@CrossOrigin(origins = "*", maxAge = 3600)
public class DeathSummary_Controller {
	
	@Autowired
	DeathSummary_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody DeathSummary_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody DeathSummary_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
