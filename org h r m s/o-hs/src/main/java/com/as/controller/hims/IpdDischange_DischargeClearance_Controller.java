...
import com.as.service.hims.IpdDischange_DischargeClearance_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/ipd-dischange-discharge_clearance")
@CrossOrigin(origins = "*", maxAge = 3600)
public class IpdDischange_DischargeClearance_Controller {
	
	@Autowired
	IpdDischange_DischargeClearance_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody IpdDischange_DischargeClearance_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody IpdDischange_DischargeClearance_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
