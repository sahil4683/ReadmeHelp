...
import com.as.service.hims.MasterOtherMasters1_DischargeSummaryTemplateMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-other_masters1-discharge_summary_template_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterOtherMasters1_DischargeSummaryTemplateMaster_Controller {
	
	@Autowired
	MasterOtherMasters1_DischargeSummaryTemplateMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterOtherMasters1_DischargeSummaryTemplateMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterOtherMasters1_DischargeSummaryTemplateMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
