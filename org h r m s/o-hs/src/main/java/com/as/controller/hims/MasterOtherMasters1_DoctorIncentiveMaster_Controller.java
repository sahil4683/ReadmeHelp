...
import com.as.service.hims.MasterOtherMasters1_DoctorIncentiveMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-other_masters1-doctor_incentive_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterOtherMasters1_DoctorIncentiveMaster_Controller {

	@Autowired
	MasterOtherMasters1_DoctorIncentiveMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterOtherMasters1_DoctorIncentiveMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterOtherMasters1_DoctorIncentiveMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
