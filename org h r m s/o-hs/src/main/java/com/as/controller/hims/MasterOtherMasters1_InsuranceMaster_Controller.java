...
import com.as.service.hims.MasterOtherMasters1_InsuranceMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/insurance_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterOtherMasters1_InsuranceMaster_Controller {
	
	@Autowired
	MasterOtherMasters1_InsuranceMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterOtherMasters1_InsuranceMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterOtherMasters1_InsuranceMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
