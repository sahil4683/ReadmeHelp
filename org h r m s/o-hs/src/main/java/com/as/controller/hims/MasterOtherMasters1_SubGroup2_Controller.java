...
import com.as.service.hims.MasterOtherMasters1_SubGroup2_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-other_masters1-sub_group2")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterOtherMasters1_SubGroup2_Controller {
	
	@Autowired
	MasterOtherMasters1_SubGroup2_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterOtherMasters1_SubGroup2_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterOtherMasters1_SubGroup2_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
