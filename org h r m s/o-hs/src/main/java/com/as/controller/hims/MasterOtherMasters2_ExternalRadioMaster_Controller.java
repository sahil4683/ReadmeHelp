...
import com.as.service.hims.MasterOtherMasters2_ExternalRadioMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-other_masters2-external_radio_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterOtherMasters2_ExternalRadioMaster_Controller {
	
	@Autowired
	MasterOtherMasters2_ExternalRadioMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterOtherMasters2_ExternalRadioMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterOtherMasters2_ExternalRadioMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
