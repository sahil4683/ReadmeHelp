...
import com.as.service.hims.MasterPackageMaster_OpdPackageMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/master-package_master-opd_package_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class MasterPackageMaster_OpdPackageMaster_Controller {
	
	@Autowired
	MasterPackageMaster_OpdPackageMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody MasterPackageMaster_OpdPackageMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody MasterPackageMaster_OpdPackageMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
