...
import com.as.service.hims.ReceptionBillOpdHealthCheckup_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/reception-bill-opdhealth_checkup")
@CrossOrigin(origins = "*", maxAge = 3600)
public class ReceptionBillOpdHealthCheckup_Controller {
	
	@Autowired
	ReceptionBillOpdHealthCheckup_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody ReceptionBillOpdHealthCheckup_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody ReceptionBillOpdHealthCheckup_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
