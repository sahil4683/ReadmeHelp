...
import com.as.service.hims.ReceptionBill_CashDues_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/reception-bill-cash_dues")
@CrossOrigin(origins = "*", maxAge = 3600)
public class ReceptionBill_CashDues_Controller {

	@Autowired
	ReceptionBill_CashDues_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody ReceptionBill_CashDues_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody ReceptionBill_CashDues_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/getDue")
	public BaseResponse getDue(Principal principal, @RequestHeader("yearCd") Long yearCd) {
		return service.getDue(yearCd);
	}
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@GetMapping("/printReport/{format}/{uhid}")
	@ResponseBody
	public HttpEntity<byte[]> printReport(@PathVariable("format") String format, @PathVariable(value = "uhid") String uhid) {
		final byte[] data = service.report_PDF(uhid,format);
