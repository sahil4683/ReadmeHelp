...
import com.as.service.patho.Patho_ExternalLabMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/patho_external_lab_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class Patho_ExternalLabMaster_Controller {
	
	@Autowired
	Patho_ExternalLabMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody Patho_ExternalLabMaster_DTO form,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody Patho_ExternalLabMaster_DTO form, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
