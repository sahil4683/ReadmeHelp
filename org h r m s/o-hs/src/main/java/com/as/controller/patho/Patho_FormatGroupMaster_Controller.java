...
import com.as.service.patho.Patho_FormatGroupMaster_Service;
import com.as.utils.ApiValidationUtility;

@RestController
@RequestMapping("/patho_format_group_master")
@CrossOrigin(origins = "*", maxAge = 3600)
public class Patho_FormatGroupMaster_Controller {
	
	@Autowired
	Patho_FormatGroupMaster_Service service;
	
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody List<Patho_FormatGroupMaster_DTO> forms,
	...
	@PreAuthorize("hasAnyRole('ADMIN','USER')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody List<Patho_FormatGroupMaster_DTO> forms, Principal principal, @RequestHeader("yearCd") Long yearCd) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(forms);
		if(response.getStatus() == 300) {
			return response;
		}
