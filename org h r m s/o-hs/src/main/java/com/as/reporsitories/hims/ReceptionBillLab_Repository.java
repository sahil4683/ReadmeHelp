...
public interface ReceptionBillLab_Repository extends JpaRepository<ReceptionBillLab_Entity, Integer> {

	ReceptionBillLab_Entity findById(Long id);
	
	@Query("SELECT MAX(cast(a.billNo as int)) FROM ReceptionBillLab_Entity a Order By a.id asc")
	String getNextBillNo();
	
//	ReceptionBillLab_Entity findTopByOrderByIdDesc();
	
	// @Query(value="SELECT a,r FROM com.as.h_entities.ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillLab_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.date=?1 ORDER BY a.date DESC")
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillLab_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.date=:date AND a.creditYear=:creditYear ORDER BY a.date DESC")
	List<Object[]> getFullByDate(String date, @Param("creditYear") Long creditYear);
	// @Query(value="SELECT a,r FROM com.as.h_entities.ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillLab_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid ORDER BY a.date DESC")
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillLab_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.creditYear=:creditYear ORDER BY a.date DESC")
	List<Object[]> getAll(@Param("creditYear") Long creditYear);
	
	ReceptionBillLab_Entity findByBillNoAndCreditYear(String billNo,Long creditYear);
	
	@Query(value="SELECT b.id,r.name as name,b.uhid as uhid,b.bill_no as billNo FROM reception_bill_lab b INNER JOIN reception_reception_registration r ON b.uhid = r.uhid "
	...
	List<ReceptionBillLab_Entity> findAll();
	
	@Query(value="SELECT SUM(a.due) FROM reception_bill_lab a WHERE CONVERT(a.due,UNSIGNED INTEGER)>0", nativeQuery = true)
	Integer getSumOfDue();
	...
	@Query(value="SELECT " + 
			...
			"RIGHT JOIN reception_bill_lab a " + 
			"ON r.uhid = a.uhid " + 
	...
	@Query(value="SELECT " + 
			...
			"FROM reception_bill_lab o, reception_bill_lab_details d, reception_reception_registration r " + 
	...
	//Patho
	List<ReceptionBillLab_Entity> findByUhid(String uhid);
	List<ReceptionBillLab_Entity> findByDateAndCreditYear(String uhid,Long creditYear);
	...
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillLab_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.id=:id AND a.creditYear=:creditYear")
