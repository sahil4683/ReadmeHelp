...
public interface ReceptionBillOpd_Repository extends JpaRepository<ReceptionBillOpd_Entity, Integer> {

	ReceptionBillOpd_Entity findById(Long id);
	
	@Query("SELECT MAX(cast(a.billNo as int)) FROM ReceptionBillOpd_Entity a Order By a.id asc")
	String getNextBillNo();
	
//	ReceptionBillOpd_Entity findTopByOrderByIdDesc();
	
	// @Query(value="SELECT a,r FROM com.as.h_entities.ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillOpd_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.date=?1 ORDER BY a.date DESC")
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillOpd_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.date=:date AND a.creditYear=:creditYear ORDER BY a.date DESC")
	List<Object[]> getFullByDate(String date, @Param("creditYear") Long creditYear);
	// @Query(value="SELECT a,r FROM com.as.h_entities.ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillOpd_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid ORDER BY a.date DESC")
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillOpd_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.creditYear=:creditYear ORDER BY a.date DESC")
	List<Object[]> getAll(@Param("creditYear") Long creditYear);
	
	ReceptionBillOpd_Entity findByBillNoAndCreditYear(String billNo,Long creditYear);
	
	@Query(value="SELECT b.id,r.name as name,b.uhid as uhid,b.bill_no as billNo FROM reception_bill_opd b INNER JOIN reception_reception_registration r ON b.uhid = r.uhid "
	...
	@Query(value="SELECT SUM(a.due) FROM reception_bill_opd a WHERE CONVERT(a.due,UNSIGNED INTEGER)>0", nativeQuery = true)
	Integer getSumOfDue();
	...
	@Query(value="SELECT " + 
			...
			"RIGHT JOIN reception_bill_opd a " + 
			"ON r.uhid = a.uhid " + 
	...
	List<ReportBillInterface> getBillReport(
		...
		@Query(value="SELECT " + 
				...
				"FROM reception_bill_opd o, reception_bill_opd_details d, reception_reception_registration r " + 
		...
		@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBillOpd_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.id=:id AND a.creditYear=:creditYear")
