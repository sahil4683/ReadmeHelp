...
public interface ReceptionBill_RefundRadio_Repository extends JpaRepository<ReceptionBill_RefundRadio_Entity, Integer> {

	ReceptionBill_RefundRadio_Entity findById(Long id);
	
	@Query("SELECT MAX(cast(a.billNo as int)) FROM ReceptionBill_RefundRadio_Entity a Order By a.id asc")
	String getNextBillNo();
	
//	ReceptionBill_RefundRadio_Entity findTopByOrderByIdDesc();
	
//	List<ReceptionBill_RefundRadio_Entity> findByDate(String date);
//	@Query(value="SELECT a,r FROM com.as.h_entities.ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBill_RefundRadio_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.date=?1 ORDER BY a.date DESC")
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBill_RefundRadio_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.date=:date AND a.creditYear=:creditYear ORDER BY a.date DESC")
	List<Object[]> getFullByDate(@Param("date") String date,@Param("creditYear") Long creditYear);
	
//	@Query(value="SELECT a,r FROM com.as.h_entities.ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBill_RefundRadio_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid ORDER BY a.date DESC")
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBill_RefundRadio_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.creditYear=:creditYear ORDER BY a.date DESC")
	...
	@Query(value="SELECT "
			...
			+ " bill.uhid AS uhid, "
			+ " bill.date AS date "
			+ " FROM reception_bill_refund_radio bill "
	...
	@Query(value="SELECT a,r FROM ReceptionReceptionRegistration_Entity r RIGHT JOIN ReceptionBill_RefundRadio_Entity a ON a.uhid = r.UHID WHERE r.UHID = a.uhid AND a.id=:id AND a.creditYear=:creditYear ORDER BY a.date DESC")
