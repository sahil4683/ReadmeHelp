...
import com.as.h_dto.hims.IpdBill_ProvisionalBill_DTO;
import com.as.h_dto.hims.IpdBill_ProvisionalBill_Details_DTO;
...
import com.as.reporsitories.hims.IpdBill_ProvisionalBill_Details_Repository;
import com.as.reporsitories.hims.IpdBill_ProvisionalBill_Repository;
...
@Service
public class IpdBill_ProvisionalBill_Service {

	@Autowired
	IpdBill_ProvisionalBill_Details_Repository details_Repository;

	@Autowired
	IpdBill_ProvisionalBill_Repository repository;
	...
	public BaseResponse save(IpdBill_ProvisionalBill_DTO form, Principal principal, Long creditYear) {
		BaseResponse response = new BaseResponse();
		IpdBill_ProvisionalBill_Entity entity = repository.findByIpdnoAndCreditYear(form.getIpdno(),creditYear);
		if (entity != null) {
		...
		entity = new IpdBill_ProvisionalBill_Entity();
		...
		for (IpdBill_ProvisionalBill_Details_DTO detail : form.getDetails()) {
			IpdBill_ProvisionalBill_Details_Entity details_Entity = new IpdBill_ProvisionalBill_Details_Entity();
	...
	private Discharge_DTO getdischargeDTO(IpdBill_ProvisionalBill_Entity entity, Long creditYear) {
	...
	public BaseResponse update(IpdBill_ProvisionalBill_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		IpdBill_ProvisionalBill_Entity entity = repository.findById(form.getId());
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
		...
		for (IpdBill_ProvisionalBill_Details_DTO detail : form.getDetails()) {
			IpdBill_ProvisionalBill_Details_Entity details_Entity = details_Repository.findById(detail.getId());
			if (details_Entity != null) {
				BeanUtils.copyProperties(detail, details_Entity);
				details_Repository.save(details_Entity);
			} else {
				details_Entity = new IpdBill_ProvisionalBill_Details_Entity();
				BeanUtils.copyProperties(detail, details_Entity);
	...
	public BaseResponse delete(long id, Principal principal, Long creditYear) {
		BaseResponse response = new BaseResponse();
		IpdBill_ProvisionalBill_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			List<IpdBill_ProvisionalBill_Details_Entity> delete = details_Repository.findByIpdBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
...
//		response.setBody(repository.findAll(Sort.by(Sort.Direction.DESC, "id")));
	...
	public BaseResponse deleteDetailById(Long id, Principal principal) {
		...
		response.setType(ResponseType.SUCCESS);
		response.setMessage("List");
		IpdBill_ProvisionalBill_Details_Entity entity = details_Repository.findById(id);
	...
	public byte[] report_PDF(String id, String format,String type,Principal principal,Long creditYear) {
		try {			
			...
			IpdBill_ProvisionalBill_Entity billList = repository.findById(Long.valueOf(id));
			List<IpdBill_ProvisionalBill_Details_Entity> tesmList = details_Repository.findByIpdBillNoAndCreditYear(String.valueOf(billList.getId()),creditYear);
...
class LabRequestResponse{
	...
	private String procedureDr1;
	private String procedureDr2;
	private String qty;
	private String rate;
	private String otherConcession;
	private String doctorAdditional;
	...
	public LabRequestResponse(Long id, String sno, String testName, String testId, String procedureDr1, String procedureDr2,
			String qty, String rate, String otherConcession, String doctorAdditional, String amount, String packageName,
			String packages, String pdate,String ptime, String requestType, String orgcode, String classCode, String groupCode, String groupName, String newSubGroupCode) {
		super();
		this.id = id;
		this.sno = sno;
		this.testName = testName;
		this.testId = testId;
		this.procedureDr1 = procedureDr1;
		this.procedureDr2 = procedureDr2;
		this.qty = qty;
		this.rate = rate;
		this.otherConcession = otherConcession;
		this.doctorAdditional = doctorAdditional;
		this.amount = amount;
		...
		this.newSubGroupCode = newSubGroupCode;
	}
}
