...
import com.as.h_dto.hims.ReceptionBillLab_DTO;
import com.as.h_dto.hims.ReceptionBillLab_Details_DTO;
...
@Service
public class ReceptionBillLab_Service {

	@Autowired
	ReceptionBillLab_Details_Repository details_Repository;

	@Autowired
	ReceptionBillLab_Repository repository;
	...
	public BaseResponse save(ReceptionBillLab_DTO form, Long yearCd, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBillLab_Entity entity = repository.findByBillNoAndCreditYear(form.getBillNo(), yearCd);
		if(entity !=null) {
		...
		entity = new ReceptionBillLab_Entity();
		 ...
		 */
		// For Bill Number
		NumberSequenceUtility utility = sequenceUtilityRepository.findByName("Lab Bill");
		...
		for (ReceptionBillLab_Details_DTO detail : form.getDetailsList()) {
			ReceptionBillLab_Details_Entity details_Entity = new ReceptionBillLab_Details_Entity();
	...
	public BaseResponse update(ReceptionBillLab_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBillLab_Entity entity = repository.findById(form.getId());
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
		...
		for (ReceptionBillLab_Details_DTO detail : form.getDetailsList()) {
			ReceptionBillLab_Details_Entity details_Entity = details_Repository.findById(detail.getId());
			if (details_Entity != null) {
				...
				details_Entity = new ReceptionBillLab_Details_Entity();
				BeanUtils.copyProperties(detail, details_Entity);
				details_Entity
						.setGroupName(group_Repository.findById(Long.valueOf(detail.getGroupName())).getGroupName());
				if (detail.getProcedureDoctor1() != null)
	...
	public BaseResponse delete(long id, Principal principal,Long creditYear) {
		BaseResponse response = new BaseResponse();
		ReceptionBillLab_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			List<ReceptionBillLab_Details_Entity> delete = details_Repository.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
	...
	public BaseResponse getDetailsById(Long id, Principal principal,Long creditYear) {
		...
		ReceptionBillLab_DTO dto = new ReceptionBillLab_DTO();
		List<ReceptionBillLab_Details_DTO> detailsList = new ArrayList<>();

		ReceptionBillLab_Entity entity = repository.findById(id);
		BeanUtils.copyProperties(entity, dto);

		List<ReceptionBillLab_Details_Entity> detail = details_Repository.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
		for (ReceptionBillLab_Details_Entity detail_temp : detail) {
			ReceptionBillLab_Details_DTO temp = new ReceptionBillLab_Details_DTO();
	...
	public BaseResponse deleteDetailById(Long id, Principal principal) {
		...
		response.setType(ResponseType.SUCCESS);
		response.setMessage("List");
		ReceptionBillLab_Details_Entity entity = details_Repository.findById(id);
	...
	public BaseResponse getDetailsByBillNo(String billNo, Principal principal,Long creditYear) {
		...
		ReceptionBillLab_DTO dto = new ReceptionBillLab_DTO();
		List<ReceptionBillLab_Details_DTO> detailsList = new ArrayList<>();
		ReceptionBillLab_Entity entity = repository.findByBillNoAndCreditYear(billNo,creditYear);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
	...
	public byte[] report_PDF(String uhid, String format,Long creditYear) {
		try {			
			...
			ReceptionBillLab_Entity dataList = repository.findById(Long.valueOf(uhid));
			ReceptionReceptionRegistration_Entity registration= registration_Repository.findByUHID(dataList.getUhid());
			List<ReceptionBillLab_Details_Entity> tempList = details_Repository.findByBillNoAndCreditYear(String.valueOf(dataList.getId()),creditYear);
			...
			parameters.put("billType", "LAB BILL");
