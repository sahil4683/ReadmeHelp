...
import com.as.h_dto.hims.ReceptionBillOpd_Details_DTO;
...
import com.as.reporsitories.hims.ReceptionBillOpd_Details_Repository;
...
@Service
public class ReceptionBillOpd_Service {

	@Autowired
	ReceptionBillOpd_Details_Repository details_Repository;

	@Autowired
	ReceptionBillOpd_Repository repository;
	...
	public BaseResponse save(ReceptionBillOpd_DTO form, Long yearCd, Principal principal) {
		BaseResponse response = new BaseResponse();
		
		ReceptionBillOpd_Entity entity = repository.findByBillNoAndCreditYear(form.getBillNo(), yearCd);
		if(entity !=null) {
		...
		entity = new ReceptionBillOpd_Entity();
		...
		NumberSequenceUtility utility = sequenceUtilityRepository.findByName("OPD Bill");
		...
		for (ReceptionBillOpd_Details_DTO detail : form.getDetailsList()) {
			ReceptionBillOpd_Details_Entity details_Entity = new ReceptionBillOpd_Details_Entity();
	...
	public BaseResponse update(ReceptionBillOpd_DTO form, Principal principal) {
		BaseResponse response = new BaseResponse();
		ReceptionBillOpd_Entity entity = repository.findById(form.getId());
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
		...
		for (ReceptionBillOpd_Details_DTO detail : form.getDetailsList()) {
			ReceptionBillOpd_Details_Entity details_Entity = details_Repository.findById(detail.getId());
			if (details_Entity != null) {
				...
				details_Entity = new ReceptionBillOpd_Details_Entity();
				BeanUtils.copyProperties(detail, details_Entity);
				details_Entity
						.setGroupName(group_Repository.findById(Long.valueOf(detail.getGroupName())).getGroupName());
				if (detail.getProcedureDoctor1() != null)
	...
	public BaseResponse delete(long id, Principal principal,Long creditYear) {
		BaseResponse response = new BaseResponse();
		ReceptionBillOpd_Entity entity = repository.findById(id);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			List<ReceptionBillOpd_Details_Entity> delete = details_Repository
	...
	public BaseResponse getDetailsById(Long id, Principal principal,Long creditYear) {
		...
		ReceptionBillOpd_DTO dto = new ReceptionBillOpd_DTO();
		List<ReceptionBillOpd_Details_DTO> detailsList = new ArrayList<>();

		ReceptionBillOpd_Entity entity = repository.findById(id);
		BeanUtils.copyProperties(entity, dto);

		List<ReceptionBillOpd_Details_Entity> detail = details_Repository.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
		for (ReceptionBillOpd_Details_Entity detail_temp : detail) {
			ReceptionBillOpd_Details_DTO temp = new ReceptionBillOpd_Details_DTO();
	...
	public BaseResponse deleteDetailById(Long id, Principal principal) {
		...
		response.setType(ResponseType.SUCCESS);
		response.setMessage("List");
		ReceptionBillOpd_Details_Entity entity = details_Repository.findById(id);
	...
	public BaseResponse getDetailsByBillNo(String billNo, Principal principal,Long creditYear) {
		...
		ReceptionBillOpd_DTO dto = new ReceptionBillOpd_DTO();
		List<ReceptionBillOpd_Details_DTO> detailsList = new ArrayList<>();
		ReceptionBillOpd_Entity entity = repository.findByBillNoAndCreditYear(billNo,creditYear);
		if (entity == null) {
			response.setStatus(300);
			response.setType(ResponseType.WARNING);
			response.setMessage("Not Found");
			...
			BeanUtils.copyProperties(entity, dto);
			List<ReceptionBillOpd_Details_Entity> detail = details_Repository
					.findByBillNoAndCreditYear(String.valueOf(entity.getId()),creditYear);
			for (ReceptionBillOpd_Details_Entity detail_temp : detail) {
				ReceptionBillOpd_Details_DTO temp = new ReceptionBillOpd_Details_DTO();
				BeanUtils.copyProperties(detail_temp, temp);
	...
	public byte[] report_PDF(String uhid, String format,Long creditYear) {
		try {
			...
			ReceptionBillOpd_Entity dataList = repository.findById(Long.valueOf(uhid));
			ReceptionReceptionRegistration_Entity registration= registration_Repository.findByUHID(dataList.getUhid());
			List<ReceptionBillOpd_Details_Entity> tempList = details_Repository.findByBillNoAndCreditYear(String.valueOf(dataList.getId()),creditYear);
			...
			parameters.put("billType", "OPD BILL");
