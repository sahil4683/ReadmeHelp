...
export class FinalIpdBillComponent implements OnInit {
  ...
  constructor(
    ...
    private service: FinalIpdBillService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle("OHIMS | Final IPD Bill");
  ...
  getNextNumberSequence():void{
    this.tokenStorageService.getNextNumberSequence('IPD Bill').subscribe(
      data => {
  ...
  calGrandTotal(): void {
    ...
    // if (this.form.lessDeposit) {
      this.form.netTotal = gTotal - Number(this.form.lessDeposit); 
    // } else {
  ...
  onDeleteTest(id,i): void {
    ...
    if (id == 0 || !this.isEdit || id === "0") {
      this.showLabContent = false;
      this.labRequestList.splice(i, 1);
      let tempList = this.labRequestList;
      this.labRequestList = tempList;
      this.showLabContent = true;
      this.fillTotal();
  ...
  getSubDepartmentList(): void {
    this.subdepartmentService.getSubDepartmentByDepartment("16").subscribe(
      data => {
      ...
      this.admissionService.getFullPatientDetailByIPD(ipdno).subscribe(
        data => {
          if (data.body == null) {
            ...
            // this.form.creditAgainst = data.body.registration.name + '(' + data.body.registration.uhid + ')';
