...
export class ConcessionComponent implements OnInit {
  ...
  constructor(private service: ConcessionService,
  ...
  ngOnInit(): void {
    this.tokenStorageService.setSessionPrivileges('hm_5_6_3');
    window.scrollTo(0, 0);
    this.onTable();
    this.getAuthNameList();
  }

  getAuthNameList(): void{
    this.masterCategoryService.getCategoryList('Authenticator_Name').subscribe(
      data => {
        this.AuthNameList = data.body;
      },
      err => {
        console.error(err);
      }
