...
import { DischargeSummaryTemplateMasterService } from 'src/app/_service/webhims/master/other_masters1/discharge-summary-template-master.service';
import { DischargeSummaryService } from 'src/app/_service/webhims/master/other_masters1/discharge-summary.service';
...
export class DischargeSummaryTemplateMasterComponent implements OnInit {
  ...
  constructor(private service: DischargeSummaryTemplateMasterService,
  ...
  ngOnInit(): void {
    this.tokenStorageService.setSessionPrivileges('hm_5_6_14');
    window.scrollTo(0, 0);
    this.onTable();
    this.getTypeList();
  }
  ...
  getTypeList(): void {
    this.dischargeSummaryService.getTypeList().subscribe(
      data => {
  ...
  onEdit(row): void {
    ...
    window.scrollTo(0, 0);
    this.form = row;
    this.form.templateType = Number(row.templateType);
    this.isEdit = true;
  }
  onDelete(id): void {
    Swal.fire({
