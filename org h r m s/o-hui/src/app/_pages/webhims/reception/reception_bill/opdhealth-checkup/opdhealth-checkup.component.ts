...
export class OpdhealthCheckupComponent implements OnInit {
  ...
  constructor(private doctorReferenceService: DoctorReferenceService,
              private service: OpdhealthCheckupService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle("OHIMS | Health Checkup Bill");
    this.tokenStorageService.setSessionPrivileges('hm_1_2_4');
    window.scrollTo(0, 0);
    this.onTable(0);
  ...
  getNextNumberSequence():void{
    this.tokenStorageService.getNextNumberSequence('HealthCheckup Bill').subscribe(
      data => {
