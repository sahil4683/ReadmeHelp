...
import { LabService } from 'src/app/_service/webhims/reception/reception_bill/lab.service';
import { LabRefundService } from 'src/app/_service/webhims/reception/reception_bill_refund/lab-refund.service';
...
export class LabRefundComponent implements OnInit {
  ...
  constructor(private doctorReferenceService: DoctorReferenceService,
              private service: LabRefundService,
              private getService: LabService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle("OHIMS | LAB Bill Refund");
    this.tokenStorageService.setSessionPrivileges('hm_1_3_3');
    window.scrollTo(0, 0);
    this.onTable(0);
  ...
  getNextNumberSequence():void{
    this.tokenStorageService.getNextNumberSequence('Lab Bill Refund').subscribe(
      data => {
      ...
      this.getService.getDetailsByBillNo(newBillNo).subscribe(
        data => {
          ...
          if (data.body == null) {
            ...
            this.form.methodOfPayment = "CASH";
            // this.getNextNumberSequence();
            this.form.labBillNo = data.body.billNo;
            let ok = 0;
