...
import { OpdRefundService } from 'src/app/_service/webhims/reception/reception_bill_refund/opd-refund.service';
...
export class OpdRefundComponent implements OnInit {
  ...
  constructor(private doctorReferenceService: DoctorReferenceService,
              private service: OpdRefundService,
              private getService: OpdBillService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle("OHIMS | OPD Bill Refund");
    this.tokenStorageService.setSessionPrivileges('hm_1_3_1');
    window.scrollTo(0, 0);
    this.onTable(0);
  ...
  getNextNumberSequence():void{
    this.tokenStorageService.getNextNumberSequence('OPD Bill Refund').subscribe(
      data => {
...
tokensAreLoading_table = false;
      ...
      this.getService.getDetailsByBillNo(newBillNo).subscribe(
        data => {
          ...
          if (data.body == null) {
            ...
            this.form.consultant2 = Number(data.body.consultant2);
            this.form.refBy1 = Number(data.body.refBy1);
            this.form.refBy2 = Number(data.body.refBy2);
            this.form.methodOfPayment = "CASH";
            // this.getNextNumberSequence();
            this.form.opdBillNo = data.body.billNo;
            let ok = 0;
  ...
  onEdit(item): void {
    ...
    this.service.getDetailsById(row.id).subscribe(
      data => {
        this.form = data.body;
        // this.form.uhid = Number(data.body.uhid)
        this.form.subDept = Number(data.body.subDept);
        this.form.organization = Number(data.body.organization);
        this.form.consultant1 = Number(data.body.consultant1);
        this.form.refBy1 = Number(data.body.refBy1);
