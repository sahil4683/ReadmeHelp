...
import { OpdhealthCheckupService } from 'src/app/_service/webhims/reception/reception_bill/opdhealth-checkup.service';
import { OpdhealthcheckupRefundService } from 'src/app/_service/webhims/reception/reception_bill_refund/opdhealthcheckup-refund.service';
...
export class OpdhealthCheckupRefundComponent implements OnInit {
  ...
  constructor(private doctorReferenceService: DoctorReferenceService,
              private service: OpdhealthcheckupRefundService,
              private getService: OpdhealthCheckupService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle("OHIMS | HealthCheckup Bill Refund");
    this.tokenStorageService.setSessionPrivileges('hm_1_3_4');
    window.scrollTo(0, 0);
    this.onTable(0);
  ...
  getNextNumberSequence():void{
    this.tokenStorageService.getNextNumberSequence('HealthCheckup Bill Refund').subscribe(
      data => {
  ...
  getBillTitle(): void{
    this.masterCategoryService.getCategoryList('H_HEADING').subscribe(
      data => {
        this.BillTitleList = data.body;
      },
      err => {
        console.error(err);
      }
...
tokensAreLoading_table = false;
  ...
  refreshPatientDetails(searchtext: string) {
      this.getService.filterUhidNameBillNo(searchtext).subscribe(
      data => {
  ...
  getDetailBy_BillNo(billNo): void {
    if (billNo != null && billNo.length >= 0 && billNo != '') {
      let newBillNo = billNo;
      newBillNo = newBillNo.split('/').join('_');
      this.getService.getDetailsByBillNo(newBillNo).subscribe(
        data => {
          // this.form = {};
          if (data.body == null) {
            Swal.fire({
            ...
            this.form.methodOfPayment = "CASH";
            // this.getNextNumberSequence();
            this.form.opdHealthCheckupBillNo = data.body.billNo;
            let ok = 0;
            data.body.detailsList.forEach(value => {
              ...
              this.opdService.getParticularsListByGroupAndOrganization(value.groupName, data.body.organization).subscribe(
                data => {
