...
import { RadioService } from 'src/app/_service/webhims/reception/reception_bill/radio.service';
import { RadioRefundService } from 'src/app/_service/webhims/reception/reception_bill_refund/radio-refund.service';
...
export class RadioRefundComponent implements OnInit {
  ...
  constructor(private doctorReferenceService: DoctorReferenceService,
              private service: RadioRefundService,
              private getService: RadioService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle("OHIMS | Radio Bill Refund");
    this.tokenStorageService.setSessionPrivileges('hm_1_3_2');
    window.scrollTo(0, 0);
    this.onTable(0);
  ...
  getNextNumberSequence():void{
    this.tokenStorageService.getNextNumberSequence('Radio Bill Refund').subscribe(
      data => {
      ...
      this.getService.getDetailsByBillNo(newBillNo).subscribe(
        data => {
          ...
          if (data.body == null) {
            ...
            this.form.methodOfPayment = "CASH";
            // this.getNextNumberSequence();
            this.form.radioBillNo = data.body.billNo;
            let ok = 0;
