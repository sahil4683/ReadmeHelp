...
import { PathoStatusMasterService } from 'src/app/_service/webpatho/patho-status-master.service';
import { ComponentReloadService } from 'src/app/_helpers/component-reload.service';

@Component({
  selector: 'app-patho-status-master',
  templateUrl: './patho-status-master.component.html',
  styleUrls: ['./patho-status-master.component.css']
})
export class PathoStatusMasterComponent implements OnInit {

  constructor(
    private service: PathoStatusMasterService,
    ...
    ngOnInit(): void {
      this.tokenStorageService.setSessionPrivileges('pm_3_12');
      window.scrollTo(0, 0);
      this.onTable();
    }
